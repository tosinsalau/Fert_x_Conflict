...
#Urea_price= read_excel("C:/Users/salau/OneDrive - Michigan State University/Research/Fert_x_Conflict/Nigerian_states/NGstatesFert.xlsx")
...
Urea_price$tr <- strftime(Urea_price$date2,format="%Y-%d")


Urea_Int= rename(Urea_Int, mon_yr = Date)
#####

trip_P=acledipe[(aclednig$event_type=="Protests"),]
trip_B=acledipe[(aclednig$event_type=="Battles"),]


# acleddelta= filter(trip, admin1 %in%  c("Abia", "Akwa Ibom", "Bayelsa",
#                                         "Cross River", "Delta", "Edo",
#                                         "Ondo", "Imo", "Rivers"))


# a1 = acleddelta[,c('actor1', "assoc_actor_1", 'actor2',"assoc_actor_2", 'year')]
a1 = trip_P[,c('actor1', "assoc_actor_1", 'actor2',"assoc_actor_2", 'mon_yr')]
a1 = trip_P[,c('actor1', "assoc_actor_1", 'actor2',"assoc_actor_2", 'mon_yr', 'notes')] #bring in notes
a2= trip_B[,c('actor1', "assoc_actor_1", 'actor2',"assoc_actor_2", 'mon_yr')]
...
for (id in match){
  a2[,id][which(grepl("Military Forces of Nigeria", a2[, id]))] ="Gov"
}

for (id in match){
  a2[,id][which(grepl("Police Forces of Nigeria ", a2[, id]))] ="Gov"
}
...
#Drop unidentified.. actually leave them cuz stil volent groups
for (id in match){
  a1 = a1[which(!grepl("Unidentified", a1[,id])),]
}

for (id in match){
  a2 = a2[which(!grepl("Unidentified", a2[,id])),]
}

#actrs cant fight temselves so remove
a1 = a1[a1$actor1!=a1$actor2,]

a2 = a2[a2$actor1!=a2$actor2,]

#filter for conflict involving group with police and military #should have 171 or 170
try2=a1 %>% 
  filter(actor1 == "Gov"| actor2== "Gov"| 
           assoc_actor_1 == "Gov"|assoc_actor_2 == "Gov")

try2a=a2 %>% 
  filter(actor1 == "Gov"| actor2== "Gov"| 
           assoc_actor_1 == "Gov"|assoc_actor_2 == "Gov")

#grepl wouldnt do this well
vec=c('tosin','tossin','toosin','tosin is expressive')
vec[grepl('tosin', vec)]

#REMOVE ROWS THAT DONT HAVE AN ASSoC ACTOR TO FIGHT WITH
protests <- try2[!c(try2$assoc_actor_1 == "", try2$assoc_actor_2 == "" ), ]

battles <- try2a[!c(try2a$assoc_actor_1 == "", try2a$assoc_actor_2 == "" ), ]

#export both
write.csv(protests,"C:/Users/salau/OneDrive - Michigan State University/IPE/Final/protests_wnotes.csv",row.names = FALSE)

write.csv(protests,"C:/Users/salau/OneDrive - Michigan State University/IPE/Final/protests.csv",row.names = FALSE)
write.csv(battles,"C:/Users/salau/OneDrive - Michigan State University/IPE/Final/battles.csv",row.names = FALSE)

#####bring back
prot_dat= read_csv("C:/Users/salau/OneDrive - Michigan State University/IPE/Final/protests.csv")
Civ_bat= read_csv("C:/Users/salau/OneDrive - Michigan State University/IPE/Final/battles_civ.csv")

#group by
# per_attack= prot_dat %>%
#   group_by(mon_yr, collab_milit, actor1, assoc_actor_1, actor2, assoc_actor_2) %>%
#   summarise(weight=sum(collab_milit)) %>% 
#   ungroup

#get for all coops with mils
...
All_Urea[is.na(All_Urea)] = 0
--------------------------------------- stopping here cuz this is wrong..
not looking at coop events by states 

#rename weight
All_Urea= rename(All_Urea, coop_events = weight)
All_bat= rename(All_bat, coop_events = weight)

#model1=glm.nb(weight ~ Urea_Intl, data=All_Urea)

model1=glm.nb(coop_events ~ subsidy, data=All_Urea) ###1
